...
import "./AccepetOrganizer.css"

export const AccepetOrganizer = () => {
  const [AccepetOrganizer, setAccepetOrganizer] = useState([]);
  const [searchAccepetOrganizer, setSearchAccepetOrganizer] = useState("");
  ...
  const handelOraganizerSearchChange = (e) => {
    setSearchAccepetOrganizer(e.target.value);
  };
  ...
  const fetchOrganizer = async () => {
    try {
      ...
      if (getOrganizer.data.AllOrganizer) {
        ...
        setAccepetOrganizer(getOrganizer.data.AllOrganizer);
      }
      else{
        setNotFound("Wait for Accept Organizer")
      }
    } catch (error) {
  ...
  const Accepeted = AccepetOrganizer.filter((Items)=>{
  ...
  return (
    <section className=".container-fluid p-0 m-0 " id="Accepted-Organizer-Main-Parent">
      <div className="p-0" id="Accepted-Organizer-Heading">
        <p>Accepeted Organizer</p>
      </div>
      <div
        className="w-100 d-flex  justify-content-end align-items-center p-3"
        id="Accepted-Organizer-Search-box"
      >
        ...
        <div className="d-flex justify-content-center align-items-center position-relative ">
          <input
            ...
            type="search"
            placeholder="search"
            value={searchAccepetOrganizer}
            onChange={handelOraganizerSearchChange}
          />
          <p className="position-absolute  " id="Accepted-Organizer-Search-Icon">
            <FaSearch />
          </p>
        </div>
      </div>
      <div className="row p-0 m-0" id="Accepted-Organizer-second-Parent">
        <div
          className="col-12 p-0 m-0"
          class="table-responsive"
          id="Accepted-Organizer-First-child"
        >
           { Accepeted.length===0? 
            < div id="Accepted-Organizer-Message" >
              <p id="page-logo"  >Wait For the User to Accepet the Organizer</p>
              </div>  
              :
          <table>
            <thead>
            ...
            {AccepetOrganizer
                .filter((Items) => {
                  return searchAccepetOrganizer.toLowerCase() === ""
                    ? Items
                    : Items.OrganizerName.toLowerCase().includes(
                        searchAccepetOrganizer
                      );
                }).filter((Items)=>{
                    return Items.status==="accepted"
                })
